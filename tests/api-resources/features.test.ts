...
describe('resource features', () => {
  ...
  test('batch: only required params', async () => {
    const responsePromise = client.features.batch({ elements: [{ id: 21154906, type: 'node' }, { id: 4589123, type: 'way' }] });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('batch: required and optional params', async () => {
    const response = await client.features.batch({ elements: [{ id: 21154906, type: 'node' }, { id: 4589123, type: 'way' }] });
  });
  ...
  test('query: request options and params are passed correctly', async () => {
    ...
    await expect(client.features.query({
    cursor: 'cursor',
    format: 'format',
    h3: 'h3',
    limit: 0,
