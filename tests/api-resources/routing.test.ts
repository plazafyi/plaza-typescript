...
describe('resource routing', () => {
  ...
  test('matrix: only required params', async () => {
    const responsePromise = client.routing.matrix({ destinations: [{ coordinates: [2.2945, 48.8584], type: 'Point' }], origins: [{ coordinates: [2.3522, 48.8566], type: 'Point' }, { coordinates: [2.3376, 48.8606], type: 'Point' }] });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('nearest: only required params', async () => {
    const responsePromise = client.routing.nearest({ geometry: { coordinates: [2.3522, 48.8566], type: 'Point' } });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
