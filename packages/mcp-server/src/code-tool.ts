...
const remoteStainlessHandler = async (
  ...
  const localClientEnvs = { PLAZA_API_KEY: requireValue(readEnv('PLAZA_API_KEY') ?? client.apiKey, 'set PLAZA_API_KEY environment variable or provide apiKey client option'), PLAZA_BASE_URL: readEnv('PLAZA_BASE_URL') ?? readEnv('PLAZA_ENVIRONMENT') ? undefined : client.baseURL ?? undefined };
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    body: JSON.stringify({
      ...
      code,
      intent,
      client_opts: { environment: (readEnv('PLAZA_ENVIRONMENT') || undefined) as any },
